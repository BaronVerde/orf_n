...
// @todo: calc from number of lod levels and heightmap size. Memory usage tises for small nodes.
// Must be power of 2.
...
/**
 * @todo: Will become obsolete once rendering occurs in view space
 * Applied to the y values of the bounding boxes on Node and QuadTree creation.
 * Else real wold data would run elsewhere.
 * Should be ~= 1/( distance between posts in meters ) for testing, 1.0 later
 */
static const float HEIGHT_FACTOR{ 0.01f };
 ...
 */
static const float LOD_LEVEL_DISTANCE_RATIO{ 2.0f };
...
static const float MORPH_START_RATIO{ 0.67f };
