...
void ComputeSingleScattering(
	float r, float mu, float mu_s, float nu, bool ray_r_mu_intersects_ground, out vec3 rayleigh, out vec3 mie) {
...
void GetRMuMuSNuFromScatteringTextureUvwz(
	in vec4 uvwz, out float r, out float mu, out float mu_s, out float nu, out bool ray_r_mu_intersects_ground ) {
	float H = sqrt( atmosphere.top_radius * atmosphere.top_radius - atmosphere.bottom_radius * atmosphere.bottom_radius );
	...
	if( uvwz.z < 0.5f ) {
		...
		mu = ( d == 0.0f * m ? -1.0f : ClampCosine( -(rho * rho + d * d) / (2.0f * r * d) ) );
		...
		mu = ( d == 0.0 * m ? 1.0f : ClampCosine( (H * H - rho * rho - d * d) / (2.0f * r * d) ) );
...
void GetRMuMuSNuFromScatteringTextureFragCoord( 
	in vec3 frag_coord, out float r, out float mu, out float mu_s, out float nu, out bool ray_r_mu_intersects_ground ) {
...
void main() {
	ComputeSingleScatteringTexture( vec3( gl_FragCoord.xy, layer + 0.5f ), delta_rayleigh, delta_mie );
