...
void Node::create( const float x, const float z, const float size, const int level,
		const orf_n::AABB *wholeMapBB, const terrain::HeightMap *heightMap, Node *allNodes, int &lastIndex ) {
	...
	m_level = level;
	m_size = size;
	float rasterSizeX = (float)heightMap->getExtent().x;
	float rasterSizeZ = (float)heightMap->getExtent().y;
