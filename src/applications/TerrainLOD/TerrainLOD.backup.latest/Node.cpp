...
void Node::create( const int x, const int z, const int size, const int level,
		const orf_n::AABB *const wholeMapBB, const terrain::HeightMap *heightMap, Node *allNodes, int &lastIndex ) {
	...
	int limitX = std::min( heightMap->getExtent().x, x + size + 1 );
	// limit z = y-axis of heightmap
	int limitZ = std::min( heightMap->getExtent().y, z + size + 1 );
	...
	omath::dvec3 min{ wholeMapBB->m_min.x + m_x, m_minMaxHeight.x, wholeMapBB->m_min.z + m_z };
	omath::dvec3 max{ wholeMapBB->m_min.x + m_x + m_size, m_minMaxHeight.y, wholeMapBB->m_min.z + m_z + m_size };
