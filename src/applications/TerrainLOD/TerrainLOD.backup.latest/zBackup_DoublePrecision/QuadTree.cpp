...
#include <applications/TerrainLOD/TerrainLOD.backup.latest/zBackup_DoublePrecision/Settings.h>
...
QuadTree::QuadTree(
	...
	for( int z{ 0 }; z < m_topNodeCountZ; ++z ) {
		...
		for( int x{ 0 }; x < m_topNodeCountX; ++x ) {
			...
			m_topLevelNodes[z][x]->create( (float)(x * m_topNodeSize), (float)(z * m_topNodeSize),
					(float)m_topNodeSize, 0, tileBoundingBox, m_heightMap, m_allNodes, nodeCounter );
		}
	}
	...
	std::ostringstream s;
	// Quad tree summary
	float sizeInMemory{ (float)m_nodeCount * ( sizeof( Node ) + sizeof( orf_n::dAABB ) ) };
