...
void TextRenderer::SetupTexture() {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, font.atlas_width, font.atlas_height, 0, GL_RED, GL_UNSIGNED_BYTE, font.data.data());
...
void TextRenderer::DrawChar(char c, int x, int y, int viewport_width, int viewport_height) {
	if (c < 0x20 || c > 0x7e)
		c = '?';
	...
	const GLfloat char_height = font.char_height;
	{
		...
		const GLfloat translate_x = (c % characters_per_line) * char_width / font.atlas_width;
		const GLfloat translate_y = (c / characters_per_line - 1) * char_height / font.atlas_height;
		const GLfloat texture_coord_from_model[] = { scale_x, 0, translate_x, 0, scale_y, translate_y };
		glUniformMatrix3x2fv(
				glGetUniformLocation(program_, "texture_coord_from_model"), 1, true, texture_coord_from_model
		);
	}
	{
		...
		glUniformMatrix4fv(glGetUniformLocation(program_, "clip_from_model"), 1, true, clip_from_model);
	}
