...
#include <applications/TerrainLOD/TerrainLOD.backup.latest/TerrainLOD.h>
...
TerrainLOD::TerrainLOD() : Renderable( "TerrainLOD" ) {
	...
	orf_n::Ellipsoid eps{ orf_n::Ellipsoid::WGS84_ELLIPSOID_SMALL };
	for( int i{0}; i < MAX_NUMBER_OF_TILES; ++i )
		m_terrainTiles[i] = new terrain::TerrainTile{ TERRAIN_FILES[i], &eps };
...
void TerrainLOD::setup() {
	...
	m_scene->getCamera()->setFarPlane( 2000.0f );
