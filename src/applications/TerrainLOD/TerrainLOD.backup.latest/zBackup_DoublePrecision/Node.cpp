...
#include <applications/TerrainLOD/TerrainLOD.backup.latest/zBackup_DoublePrecision/LODSelection.h>
#include <applications/TerrainLOD/TerrainLOD.backup.latest/zBackup_DoublePrecision/Node.h>
...
void Node::create( const float x, const float z, const float size, const int level,
		const orf_n::dAABB *const wholeMapBB, const terrain::HeightMap *heightMap, Node *allNodes, int &lastIndex ) {
	...
	m_level = level;
	m_size = size;
	const float rasterSizeX{ static_cast<float>( heightMap->getExtent().x ) };
	const float rasterSizeZ{ static_cast<float>( heightMap->getExtent().y ) };
	// Find min/max heights at this patch of terrain
	float limitX = std::min( rasterSizeX, x + size + 1 );
	float limitZ = std::min( rasterSizeZ, z + size + 1 );
	...
	omath::vec3 min{ wholeMapBB->m_min.x + m_x, m_minMaxHeight.x * HEIGHT_FACTOR, wholeMapBB->m_min.z + m_z };
	omath::vec3 max{ wholeMapBB->m_min.x + m_x + m_size, m_minMaxHeight.y * HEIGHT_FACTOR, wholeMapBB->m_min.z + m_z + m_size };
	m_boundingBox = new orf_n::dAABB{ min, max };
	// Are we done already ?
	if( size == LEAF_NODE_SIZE ) {
		...
		m_subTL->create( x, z, subSize, level+1, wholeMapBB, heightMap, allNodes, lastIndex );
		if( ( x + subSize ) < rasterSizeX ) {
	         m_subTR = &allNodes[lastIndex++];
	         m_subTR->create( x + subSize, z, subSize, level+1, wholeMapBB, heightMap, allNodes, lastIndex );
		}
		if( (z + subSize) < rasterSizeZ ) {
			m_subBL = &allNodes[lastIndex++];
			m_subBL->create( x, z + subSize, subSize, level+1, wholeMapBB, heightMap, allNodes, lastIndex );
		}
		if( ( ( x + subSize ) < rasterSizeX ) && ( ( z + subSize ) < rasterSizeZ ) ) {
			m_subBR = &allNodes[lastIndex++];
			m_subBR->create( x + subSize, z + subSize, subSize, level+1, wholeMapBB, heightMap, allNodes, lastIndex );
		}
	}
}
...
const orf_n::dAABB *Node::getBoundingBox() const {
...
orf_n::intersect_t Node::lodSelect( LODSelection *lodSelection, bool parentCompletelyInFrustum ) {
	...
	if( !( removeSubTL && removeSubTR && removeSubBL && removeSubBR ) &&
		...
		if( !lodSelection->m_isVisibilityDistanceTooSmall && (m_level != 0 ) ) {
			float maxDistFromCam = std::sqrt( m_boundingBox->maxDistanceFromPointSq( cam->getPosition() ) );
			float morphStartRange = lodSelection->m_morphStart[lodSelection->m_stopAtLevel - m_level + 1];
			if( maxDistFromCam > morphStartRange )
				lodSelection->m_isVisibilityDistanceTooSmall = true;
		}
