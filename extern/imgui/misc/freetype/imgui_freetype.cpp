...
bool ImGuiFreeType::BuildFontAtlas(ImFontAtlas* atlas, unsigned int extra_flags)
{
    ...
    float min_rects_per_row = std::ceil( ( (float)atlas->TexWidth / ( max_glyph_size.x + 1.0f ) ) );
    float min_rects_per_column = std::ceil( (float)total_rects / min_rects_per_row );
    atlas->TexHeight = (int)(min_rects_per_column * (max_glyph_size.y + 1.0f));

    // Create texture
    atlas->TexHeight = (atlas->Flags & ImFontAtlasFlags_NoPowerOfTwoHeight) ? (atlas->TexHeight + 1) : ImUpperPowerOfTwo(atlas->TexHeight);
    atlas->TexUvScale = ImVec2( 1.0f / (float)atlas->TexWidth, 1.0f / (float)atlas->TexHeight );
