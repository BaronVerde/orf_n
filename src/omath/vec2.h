...
#pragma GCC diagnostic pop
	...
	vec2_t( T const& scalar ) : x{scalar}, y{scalar} {}
...
template <typename T>
inline vec2_t<T> operator-( vec2_t<T> const& v ) {
	return vec2_t<T>{ -v.x, -v.y };
}

// Binary ops
template<typename T>
inline vec2_t<T> operator+( vec2_t<T> const& v, T const scalar ) {
	return vec2_t<T>{ v.x + scalar, v.y + scalar };
}

template<typename T>
inline vec2_t<T> operator+( T const scalar, vec2_t<T> const& v ) {
	return vec2_t<T>{ scalar + v.x,	scalar + v.y };
}

template<typename T>
inline vec2_t<T> operator+( vec2_t<T> const& v1, vec2_t<T> const& v2 ) {
...
template<typename T>
inline vec2_t<T> operator/(T scalar, vec2_t<T> const& v ) {
	return vec2_t<T>( scalar / v.x,	scalar / v.y);
}
...
// Boolean ops
template<typename T>
inline bool operator==( vec2_t<T> const& v1, vec2_t<T> const& v2 ) {
...
// Geometry
template <typename T>
static inline T dot( vec2_t<T> const& l, vec2_t<T> const& r ) {
...
template <typename T>
static inline T magnitude( vec2_t<T> const& l ) {
	return std::sqrt( dot( l, l ) );
}

template <typename T>
static inline T magnitudeSq( vec2_t<T> const& l ) {
	return dot( l, l );
}

template <typename T>
static inline T distance( vec2_t<T> const& l, vec2_t<T> const& r ) {
	return magnitude( l - r );
}

template <typename T>
static inline T distanceSq( vec2_t<T> const& l, vec2_t<T> const& r ) {
	return magnitudeSq( l - r );
}

template <typename T>
static inline vec2_t<T> normalize( vec2_t<T> const& v ) {
	if( !std::numeric_limits<T>::is_iec559 )
...
template <typename T>
static inline T angle( vec2_t<T> const& l, vec2_t<T> const& r ) {
	return std::acos( dot( l, r ) / std::sqrt( magnitudeSq(l) * magnitudeSq(r ) ) );
}

template <typename T>
static inline vec2_t<T> project( vec2_t<T> const& length, vec2_t<T> const& direction ) {
	return direction * ( dot( length, direction ) / magnitudeSq(direction) );
}

template <typename T>
static inline vec2_t<T> perpendicular( vec2_t<T> const&  length, vec2_t<T> const& direction ) {
	return length - project( length, direction );
}

template <typename T>
static inline vec2_t<T> reflection( vec2_t<T> const& sourceVector, vec2_t<T> const& normal ) {
...
template <typename T>
std::ostream &operator<<( std::ostream &o, vec2_t<T> const& v ) {
	o << '(' << v.x << '/' << v.y << ')';
	return o;
}
