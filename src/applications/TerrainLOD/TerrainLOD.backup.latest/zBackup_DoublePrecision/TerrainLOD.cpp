...
#include <applications/TerrainLOD/TerrainLOD.backup.latest/zBackup_DoublePrecision/QuadTree.h>
#include <applications/TerrainLOD/TerrainLOD.backup.latest/zBackup_DoublePrecision/TerrainLOD.h>
#include <applications/TerrainLOD/TerrainLOD.backup.latest/zBackup_DoublePrecision/TerrainTile.h>
...
TerrainLOD::TerrainLOD() : Renderable( "TerrainLOD" ) {
	...
	modules.push_back( std::make_shared<orf_n::Module>( GL_VERTEX_SHADER,
			"Source/Applications/TerrainLOD/Terrain.vert.glsl" ) );
	modules.push_back( std::make_shared<orf_n::Module>( GL_FRAGMENT_SHADER,
			"Source/Applications/TerrainLOD/Terrain.frag.glsl" ) );
...
void TerrainLOD::setup() {
	...
	m_scene->getCamera()->setPositionAndTarget( { 2048.0, 100.0, 0.0 }, { 2048.0, 0.0, 2048.0 } );
...
void TerrainLOD::render() {
	...
	//orf_n::setUniform( m_shaderTerrain->getProgram(), "g_cameraPos", omath::vec3{ m_scene->getCamera()->getPosition() } );
	orf_n::setCameraPosition( m_scene->getCamera()->getPosition() );
	...
	for( size_t i{0}; i < m_terrainTiles.size(); ++i ) {
		...
		const orf_n::dAABB *const aabb{ m_terrainTiles[i]->getAABB() };
		orf_n::setUniform( m_shaderTerrain->getProgram(), "g_tileWorldMax", omath::vec2{ aabb->m_max.x, aabb->m_max.z } );
	...
	for( size_t i{0}; i < m_terrainTiles.size(); ++i ) {
		...
		if( m_showSelectedBoxes ) {
			for( int i{ 0 }; i < m_lodSelection->m_selectionCount; ++i ) {
				...
				if( drawFull ) {
					orf_n::dAABB b{ n.node->getBoundingBox()->expand( -0.003f ) };
					m_drawPrimitives.drawAABB( b, orf_n::color::rainbow[n.node->getLevel()] );
				} else {
					if( n.hasTL ) {
						orf_n::dAABB b{ n.node->getUpperLeft()->getBoundingBox()->expand( -0.002f ) };
						m_drawPrimitives.drawAABB( b, orf_n::color::rainbow[n.node->getUpperLeft()->getLevel()] );
					}
					if( n.hasTR ) {
						orf_n::dAABB b{ n.node->getUpperRight()->getBoundingBox()->expand( -0.002f ) };
						m_drawPrimitives.drawAABB( b, orf_n::color::rainbow[n.node->getUpperRight()->getLevel()] );
					}
					if( n.hasBL ) {
						orf_n::dAABB b{ n.node->getLowerLeft()->getBoundingBox()->expand( -0.002f ) };
						m_drawPrimitives.drawAABB( b, orf_n::color::rainbow[n.node->getLowerLeft()->getLevel()] );
					}
					if( n.hasBR ) {
						orf_n::dAABB b{ n.node->getLowerRight()->getBoundingBox()->expand( -0.002f ) };
...
void TerrainLOD::debugDrawLowestLevelBoxes( const terrain::TerrainTile *const t ) const {
	...
	for( int i{ 0 }; i < t->getQuadTree()->getNodeCount(); ++i )
		if( nodes[i].getLevel() == terrain::NUMBER_OF_LOD_LEVELS - 1 ) {
			const orf_n::dAABB box{ *( nodes[i].getBoundingBox() ) };
