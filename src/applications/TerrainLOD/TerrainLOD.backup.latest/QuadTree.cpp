...
QuadTree::QuadTree(
		const terrain::HeightMap *const heightMap,
		const omath::dvec3 &min,
		const omath::dvec3 &max,
		const orf_n::AABB *const tileBoundingBox ) {

	if( heightMap->getExtent().x > 65535 || heightMap->getExtent().y > 65535 ) {
	...
	m_rasterSizeX = heightMap->getExtent().x;
	m_rasterSizeZ = heightMap->getExtent().y;
	/*float leafNodeWorldSizeX = (float)LEAF_NODE_SIZE * (float)( max.x - min.x ) / (float)m_rasterSizeX;
	float leafNodeWorldSizeZ = (float)LEAF_NODE_SIZE * (float)( max.z - min.z ) / (float)m_rasterSizeZ;
	float nodeDiagonalSizes[NUMBER_OF_LOD_LEVELS];
	nodeDiagonalSizes[0] = std::sqrt( leafNodeWorldSizeX * leafNodeWorldSizeX +
			leafNodeWorldSizeZ * leafNodeWorldSizeZ );*/
	int totalNodeCount = 0;
	m_topNodeSize = LEAF_NODE_SIZE;
	for( int i{ 0 }; i < NUMBER_OF_LOD_LEVELS; i++ ) {
		if( i != 0 ) {
			m_topNodeSize *= 2;
			//nodeDiagonalSizes[i] = 2 * nodeDiagonalSizes[i-1];
		}
	...
	for( int z{ 0 }; z < m_topNodeCountZ; ++z ) {
		...
		for( int x{ 0 }; x < m_topNodeCountX; ++x ) {
			...
			m_topLevelNodes[z][x]->create( x * m_topNodeSize, z * m_topNodeSize,
					m_topNodeSize, 0, tileBoundingBox, heightMap, m_allNodes, nodeCounter );
		}
	}
