...
void Node::create( const int x, const int z, const int size, const int level,
	...
	const omath::dvec3 min{
		terrainTile->getAABB()->m_min.x + m_x,
		m_minMaxHeight.x,
		terrainTile->getAABB()->m_min.z + m_z
	};
	const omath::dvec3 max{
		terrainTile->getAABB()->m_min.x + m_x + m_size,
		m_minMaxHeight.y,
		terrainTile->getAABB()->m_min.z + m_z + m_size
	};
	m_tileBoundingBox = new orf_n::AABB{ min, max };
	// Calculate the world bounding box in cartesian coords from geodetic positions and 0 height
	const orf_n::Geodetic geodeticMin{
		omath::radians( terrainTile->getLatLon().x + m_z * terrainTile->getCellSize() ),
		omath::radians( terrainTile->getLatLon().y + m_x * terrainTile->getCellSize() ),
		0.0
	};
	const omath::dvec3 worldMin{ terrainTile->getEllipsoid()->toCartesian( geodeticMin ) };
	const omath::dvec3 worldMax{
		terrainTile->getEllipsoid()->toCartesian( { /*orf_n::Geodetic*/
			omath::radians( terrainTile->getLatLon().x + ( m_z + m_size ) * terrainTile->getCellSize() ),
			omath::radians( terrainTile->getLatLon().y + ( m_x + m_size ) * terrainTile->getCellSize() ),
			0.0
		} )
	};
	m_worldBoundingBox = new orf_n::AABB{ worldMin, worldMax };
	m_geodeticSurfaceNormal = terrainTile->getEllipsoid()->geodeticSurfaceNormal( geodeticMin );
...
const orf_n::AABB *Node::getWorldBoundingBox() const {
	return m_worldBoundingBox;
}

const orf_n::AABB *Node::getTileBoundingBox() const {
	return m_tileBoundingBox;
}

const omath::vec3 &Node::getGeodeticSurfaceNormal() const {
	return m_geodeticSurfaceNormal;
}
...
const omath::ivec2 Node::getXZ() const {
	return omath::ivec2{ m_x, m_z };
}
...
orf_n::intersect_t Node::lodSelect( LODSelection *lodSelection, bool parentCompletelyInFrustum ) {
	...
	orf_n::intersect_t frustumIntersection = parentCompletelyInFrustum ?
			orf_n::INSIDE : cam->getViewFrustum().isBoxInFrustum( m_worldBoundingBox );
	if( orf_n::OUTSIDE == frustumIntersection )
		return orf_n::OUTSIDE;
	float distanceLimit = lodSelection->m_visibilityRanges[m_level];
	if( !m_worldBoundingBox->intersectSphereSq( cam->getPosition(), distanceLimit * distanceLimit ) )
	...
	if( m_level != lodSelection->m_stopAtLevel ) {
		...
		if( m_worldBoundingBox->intersectSphereSq( cam->getPosition(), nextDistanceLimit * nextDistanceLimit ) ) {
	...
	if( !( removeSubTL && removeSubTR && removeSubBL && removeSubBR ) &&
		...
		if( lodSelection->m_sortByDistance )
			lodSelection->m_selectedNodes[lodSelection->m_selectionCount].minDistanceToCamera =
					...
					node->getWorldBoundingBox()->minDistanceFromPointSq( cam->getPosition() ) );
